...
def generate_synthetic_mail(data: InputData):
    """
    ...
    for url in data.last_3_pages_visited:
        last_visited_pages[f"{str(url)}"] = intent_email.extract_headings_and_paragraphs(str(url))
    ...
    intent_prompt = intent_prompt_generator.generate_intent_prompt_template()
    
    # Generate the intent messages using the generated intent prompt
    intent_user_messages = intent_prompt_generator.intent_messages(intent_prompt)
    
    # Use the intent_email object to predict the intent using the generated messages
    intent_result = intent_email.intent_prediction(intent_user_messages)
    ...
    email_prompt = email_prompt_generator.generate_email_prompt_template()
    
    # Generate the email messages using the generated email prompt
    email_user_message = email_prompt_generator.email_messages(email_prompt)

    # Use the intent_email object to predict the email content using the generated email messages
    synthetic_email = intent_email.email_prediction(email_user_message)
